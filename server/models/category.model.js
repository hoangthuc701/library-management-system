...
module.exports = {
    ...
    loadName: function (name) {
        return db.load(`select * from ${TBL_CATEGORY} where name = '${name}'`)
    },
    ...
    loadByOffset: (offset) =>{
        return db.load(`SELECT * FROM ${TBL_CATEGORY} order by UNIX_TIMESTAMP(created_at) DESC LIMIT 10 OFFSET ${offset}`)
    }
}
